    ...
    try:
        if path.endswith('.geo.json'):
            with open(path, 'r') as json_file:
                ...
                return {"Latitude": tempJson["location"]["lat"], "Longitude": tempJson["location"]["lng"], "Altitude": tempJson["location"]["alt"], "Updated": d.strftime('%Y-%m-%dT%H:%M:%S.%f')}
        else:
            with open(path, 'r') as json_file:
...
def _transform_wigle_entry(gps_data, pcap_data, plugin_version):
    ...
    required_gps_fields = ['Latitude', 'Longitude', 'Altitude', 'Updated']
    if all(field in gps_data for field in required_gps_fields):
        writer.writerow([
        ...
        logging.debug(f"WIGLE: Missing required GPS fields in data: {gps_data}")
